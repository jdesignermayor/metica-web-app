...
import { CommentType, useAppStore } from "@store/store";
...
export default function ChatAICreator() {
    ...
    const [firstComment, setFirstComment] = useState<CommentType>();
    const ref = useRef<HTMLDivElement>(null);
    const { comments, flags: { isRefreshingSuggestions }, getSuggestions } = useAppStore();
    ...
    return (
        <div>
            ...
            <div className=" bg-[#121212] min-w-[450px] rounded-2xl px-2 py-2 h-[620px] overflow-x-auto my-4 divide-y divide-[#252525]" style={{ colorScheme: 'dark' }}>
                {
                    comments && comments.length > 0 && comments.map(({ id, message, isSuggest, suggestions }) => {
                        return (
                            <div key={id}>
                                ...
                                {isSuggestionsReady && isSuggest && suggestions && suggestions.length > 0 && <div className="flex flex-wrap gap-3 mt-4">
